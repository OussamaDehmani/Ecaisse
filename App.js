...
export default function App() {
  ...
  useEffect(() => {
    ...
    // });
    db.transaction((tx) => {
      tx.executeSql("DROP TABLE IF EXISTS categories", [], (tx, results) => {
        // console.log(`get result ${results}`);
      });
    });
    ...
    db.transaction((tx) => {
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT,uri TEXT)"
      );
    });

    db.transaction((tx) => {
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS sub_categories1 (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT,idCategory INTEGER,prix_detail INTEGER,prix_gros INTEGER,quantity INTEGER)"
      );
    });

    db.transaction((tx) => {
      tx.executeSql(
        "CREATE TABLE IF NOT EXISTS commandes1 ( id INTEGER PRIMARY KEY AUTOINCREMENT,date datetime default current_timestamp, name TEXT,idSubCategory INTEGER,prix_detail INTEGER,prix_gros INTEGER,quantity INTEGER)"
      );
    });
  }, []);
